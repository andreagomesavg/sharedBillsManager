      ...
      if (isLogin) {
        ...
        if (modoRegistro === 'nuevo') {
          ...
          const { data: grupo, error: errorGrupo } = await supabase
            .from('grupos')
        .select('id')
        .ilike('codigo_invitacion', codigoInvitacion.trim()) // .ilike ignora mayúsculas y .trim() borra espacios invisibles
        .single()
      ...
      <div className="bg-white max-w-md w-full p-8 rounded-3xl shadow-xl border border-gray-100">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-gray-900 mb-2">
            {isLogin ? '¡Hola de nuevo! 👋' : 'Comienza a compartir'}
          </h1>
          <p className="text-gray-500">
            {isLogin ? 'Inicia sesión en tu casa' : 'Crea tu cuenta para gestionar gastos'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button type="submit" disabled={cargando} className="w-full bg-gray-900 text-white font-bold py-4 rounded-xl hover:bg-gray-800 transition shadow-lg mt-6 active:scale-95">
            {cargando ? 'Cargando...' : (isLogin ? 'Entrar' : 'Registrarme')}
          </button>
        </form>
